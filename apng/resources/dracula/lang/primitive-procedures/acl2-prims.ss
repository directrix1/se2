...
Other library functions are implemented in (file "acl2-prims-scheme.ss").
|#
#lang scheme

(require "../dracula-core.ss"
         "../../private/planet.ss"
         (prefix-in srfi: srfi/1)
         (prefix-in mz: scheme)
         "acl2-prims-scheme.ss")

(require (for-syntax (cce text)))

(provide (except-out (all-from-out "acl2-prims-scheme.ss")
                     define/c
                     not-nil))

(define (false->nil x) (mz:if x x nil))

(define (scheme->CL x) (mz:if x (mz:if (mz:eq? x #t) t x) nil))
